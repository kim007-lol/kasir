...
@push('scripts')
<script>
    // ===== SMART NOTIFICATION SYSTEM (Dashboard) =====
        ...
        fetch('{{ route("cashier.bookings.pendingCount") }}')
            ...
            .then(data => {
                ...
                if (data.pending_count > 0) {
                    alert.style.display = '';
                    countEl.textContent = data.pending_count;
                    if (menuBadge) { menuBadge.textContent = data.pending_count; menuBadge.style.display = ''; }
                } else {
                ...
                if (data.pending_count > lastPendingCount) {
                    ...
                    if (typeof toastr !== 'undefined') {
                        toastr.info('Ada pesanan baru masuk!', 'Pesanan Baru');
                    }
                }

                lastPendingCount = data.pending_count;
            })
            .catch(err => console.log('Dashboard polling error:', err));
    }

    function startPolling(interval) {
        if (pollInterval) clearInterval(pollInterval);
        pollInterval = setInterval(pollBookingDashboard, interval);
    }

    pollBookingDashboard();
        ...
        if (window.Echo) {
            ...
            window.Echo.channel('bookings')
                .listen('NewBookingCreated', (e) => {
                    console.log('⚡ Dashboard: Real-time booking received:', e.booking);

                    const alert = document.getElementById('booking-alert');
                    const countEl = document.getElementById('pending-count');
                    const menuBadge = document.getElementById('menu-badge');

                    let newCount = parseInt(countEl.textContent || '0') + 1;
                    countEl.textContent = newCount;
                    alert.style.display = '';

                    if (menuBadge) { menuBadge.textContent = newCount; menuBadge.style.display = ''; }
